...
# قراءة قائمة المستخدمين من Variable
...
for user in users_list:
    ...
    notif_email = user.get('email', '').strip()  # ← الإيميل من USERS_CONFIG فقط
    ...
    for t in upcoming:
        ...
        rows += (
            ...
            f'<td style="padding:12px;border-bottom:1px solid #e5e7eb;color:#6366f1;font-weight:700">{days_label}</td>'
            ...
            f'<td style="padding:12px;border-bottom:1px solid #e5e7eb;font-family:monospace">{t.get("pnr","—") or "—"}</td>'
            '</tr>'
        )
    ...
    html = (
        ...
        f'<div style="font-size:16px;font-weight:800;color:#1e3a5f;margin-bottom:16px">لديك {len(upcoming)} رحلة قادمة</div>'
        ...
        '<th style="padding:12px;text-align:right">متى</th>'
